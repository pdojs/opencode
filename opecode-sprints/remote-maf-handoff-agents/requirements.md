...
- Running `/agent` in OpenCode's prompt opens a dialog listing both local subagents and any configured remote MAF containers (queried live via `GET /agents/manifest`), each remote entry showing the container's declared orchestrator name/description.
