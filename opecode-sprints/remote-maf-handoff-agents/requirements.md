...
7. Participant targeting: since MAF's handoff routing is entirely tool-call-decided by whichever agent is currently active (no `HandoffBuilder`/`Workflow` force-override API exists — confirmed via `_handoff.py`), "select ANY agent to steer or stop" is implemented as a new `steer_to_agent` wire frame that injects an advisory, user-visible instruction for the active agent to hand off, plus reuse of the existing session interrupt for "stop." This is explicitly best-effort, not a hard override, and is surfaced to the user as such in the picker UI copy — see design-proposal.md WS1/WS2/WS4.
...
- From a bound remote session, the user can open a participant list (`/participants`) and either steer toward or stop any specific participant agent in the handoff group — steering is observably best-effort (the targeted agent becomes active in most cases given cooperative sample-agent prompts, but is not a guaranteed override), and stopping a specific participant is achieved via the existing session interrupt since only one participant is ever active at a time.
...
See `design-proposal.md` for investigation, root cause, and per-workstream resolution, and `wbs.md` for the dependency graph, branch names, and implementation order. See `test-requirements.md` for the three-tier demo scripts used to validate this Definition of Done end-to-end.
